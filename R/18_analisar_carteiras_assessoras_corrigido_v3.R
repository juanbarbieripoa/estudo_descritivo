...
# Algumas variáveis administrativas já estão presentes no produto do
# módulo 17. Antes do cruzamento, validamos os campos sobrepostos e
# adicionamos somente as colunas que ainda não existem no índice. Isso
# evita que o `left_join()` crie sufixos `.x` e `.y` e remova o nome
# canônico esperado nas etapas seguintes.

colunas_perfil_desejadas <- c(
  "tipo_vinculo_rede_final",
  "status_rede_2025_final",
  "municipalizada_apos_2024",
  "possivel_municipalizacao_recente",
...
colunas_sobrepostas <- intersect(
  colunas_perfil_desejadas,
  names(indice_escola)
)

if (length(colunas_sobrepostas) > 0) {
  diagnostico_sobreposicao <- indice_escola |>
    select(
      id_escola,
      all_of(colunas_sobrepostas)
    ) |>
    rename_with(
      ~ paste0(.x, "__indice"),
      all_of(colunas_sobrepostas)
  ...
  divergencias_sobreposicao <- map_lgl(
    colunas_sobrepostas,
    function(variavel) {
      x <- diagnostico_sobreposicao[[
        paste0(variavel, "__indice")
      ]]
      y <- diagnostico_sobreposicao[[
        paste0(variavel, "__perfil")
      ]]
  ...
  if (any(divergencias_sobreposicao)) {
    stop(
      "Há divergências entre índice e perfil nas variáveis sobrepostas: ",
      paste(
        colunas_sobrepostas[divergencias_sobreposicao],
        collapse = ", "
      )
    )
  }
...
colunas_perfil_complementares <- setdiff(
  colunas_perfil_desejadas,
  names(indice_escola)
)
...
perfil_complementar <- perfil_escola |>
  select(
    id_escola,
    all_of(colunas_perfil_complementares)
  )
...
analise_carteiras <- analise_carteiras |>
  mutate(
    ...
    participacao_matriculas_rede_pct = {
      denominador <- totais_rede$total_matriculas_rede[[1]]

      if (
        is.finite(denominador) &&
          denominador > 0
      ) {
        100 * matriculas_anos_iniciais_total / denominador
      } else {
        rep(NA_real_, n())
      }
    },
    participacao_turmas_rede_pct = {
      denominador <- totais_rede$total_turmas_rede[[1]]

      if (
        is.finite(denominador) &&
          denominador > 0
      ) {
        100 * turmas_anos_iniciais_total / denominador
      } else {
        rep(NA_real_, n())
      }
    },
    participacao_carga_potencial_rede_pct = {
      denominador <- totais_rede$total_carga_potencial_rede[[1]]

      if (
        is.finite(denominador) &&
          denominador > 0
      ) {
        100 * carga_potencial_total / denominador
      } else {
        rep(NA_real_, n())
      }
    },
