...
limpar_observacoes_composicao <- function(x) {
  # A função recebe um único elemento, mas faz coerção explícita para
  # evitar falhas quando a coluna vier de RDS com classe incomum.
  if (length(x) == 0 || is.null(x) || all(is.na(x))) {
    return(NA_character_)
  }

  texto <- paste(as.character(x), collapse = " | ")
  texto <- trimws(texto)
  ...
  # Usa apenas funções base para evitar conflitos de namespace com
  # funções homônimas carregadas por outros pacotes.
  partes <- unlist(
    strsplit(texto, "\\s*\\|\\s*", perl = TRUE),
    use.names = FALSE
  )

  partes <- unique(trimws(partes))
...
base_fichas <- perfil_escola |>
  ...
  mutate(
    observacoes_composicao_limpa = vapply(
      as.list(observacoes_composicao),
...
validacao_final <- tribble(
  ...
  as.character(n_distinct(base_fichas_saida$id_escola)),
  ...
  as.character(nrow(base_fichas_saida)),
  as.character(nrow(perfil_escola)),
  "Erro",

  "Uma ficha HTML por escola",
  sum(manifesto_fichas$html_existe) == nrow(perfil_escola),
  as.character(sum(manifesto_fichas$html_existe)),
  as.character(nrow(perfil_escola)),
  "Erro",
  ...
  as.character(ifelse(
    file.exists(arquivo_html_compilado),
    file.info(arquivo_html_compilado)$size,
    0
  )),
  "> 0 bytes",
  "Erro",
  ...
  as.character(nrow(fichas_nao_geradas)),
  "0",
  "Erro",
  ...
  all(
    base_fichas_saida$indice_carga_potencial >= 0 &
  ...
  TRUE,
  as.character(ifelse(status_pdf$pdf_gerado, "Gerado", "Não gerado")),
  "Não bloqueante",
  "Informativo"
) |>
  mutate(
